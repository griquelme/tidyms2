...
from tidyms2.chem.exceptions import IsotopeError
...
def test_tokenize_element_valid_input(formula_str, ind, new_ind, element):
    token, test_index = formula._tokenize_element(formula_str, ind, PTABLE)
    assert test_index == new_ind
    isotope = PTABLE.get_element(element).monoisotope
...
def test_tokenize_isotope_valid_input(formula_str, ind, isotope_str, new_ind):
    token, test_index = formula._tokenize_isotope(formula_str, ind, PTABLE)
    isotope = PTABLE.get_isotope(isotope_str)
    assert test_index == new_ind
    assert isotope in token
...
def test_tokenize_formula(f_str, composition):
    ...
    test_composition = formula._parse_formula(f_str, PTABLE)
...
@pytest.mark.parametrize("charge,charge_str", [(1, "+"), (2, "2+"), (-1, "-"), (-4, "4-")])
...
def test_formula_from_dictionary():
    ...
    for k in composition:
        assert PTABLE.get_isotope(k) in f.composition
...
def test_formula_from_dictionary_invalid_isotope_type():
    ...
    with pytest.raises(ValueError):
        formula.Formula(composition, charge)  # type: ignore
